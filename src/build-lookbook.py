...
nop = nop.replace('<p class="foot">РРЦ — рекомендована роздрібна ціна на obiimy.world. Оптові умови — в прайсі за запитом.</p>', '<p class="foot">Ціни й умови — у вашого менеджера проєкту.</p>')
nop = nop.replace('<title>Obiimy — лукбук для партнерів 2026</title>', '<title>Obiimy — корпоративні подарунки 2026</title>')
nop = nop.replace('<h1>Лукбук<br>для партнерів</h1>', '<h1>Корпоративні<br>подарунки</h1>').replace('На полиці видно з іншого кінця залу.', 'Її впізнають ще до того, як відкриють.')
nop = _re.sub(r'<p><b>Дозамовлення поштучно\.</b>.*?</p>', '', nop)
nop = _re.sub(r'<p><b>Для консультантів\.</b>.*?</p>', '<p><b>Листівка від компанії.</b> У кожній коробці — листівка з вашим привітанням; підпис від руки — за бажанням.</p><p><b>Доставка.</b> В офіс однією посилкою або кожному адресату.</p>', nop)
nop = nop.replace('<p class="eb">Упаковка і сервіс</p><h2>Готова вітрина</h2>', '<p class="eb">Упаковка і догляд</p><h2>Фірмова жовта коробка</h2>')
nop = _re.sub(r'<p><b>Листівка від компанії\.</b>.*?</p><p><b>Доставка\.</b>.*?</p>', '', nop)
nop = nop.replace('Рейтинг покупців на obiimy.world — 5,0.', 'Рейтинг покупців — 5,0.')
i = nop.index('<p class="eb">Співпраця</p>'); j = nop.index('</div>', nop.index('<p class="contact">', i)) 
nop = nop[:i] + '<p class="eb">Подарунок</p><h2>Від вашої компанії</h2><p>Фірмова жовта коробка з тонким папером, листівка з вашим привітанням, відправка в офіс або кожному адресату.</p><p>Шовкова хустка живе в гардеробі роками й щоразу нагадує, хто її подарував.</p><p class="contact"><b>Ваш менеджер проєкту</b><br>____________________<br>____________________</p>' + nop[j:]
