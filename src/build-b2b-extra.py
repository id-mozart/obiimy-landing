...
def agencies():
    body = hero("Для event-, HR- і подарункових агенцій", "Шовк, який легко продати вашому клієнту",
        "Ви ведете клієнта — ми даємо продукт, матеріали й логістику: лукбук для презентації, зразки, фірмова коробка з листівкою клієнта, відправка кожному адресату.<span class=\"m-hide\"> Один контакт для всіх ваших проєктів.</span>",
        "Стати партнером", "Лукбук для клієнта, без цін (PDF)", "lookbook-obiimy-2026-noprice.pdf",
        ...
        ("Листівка", "З привітанням клієнта в кожній коробці"),
  ...
  <section class="block alt" id="range"><div class="wrap">
    ...
    <div class="grid4">
      <div class="card photo">{img("photo/scrunchie.jpg", "Резинка у фірмовій коробочці", sizes="(max-width: 640px) 50vw, 25vw")}<div class="in"><h3>Резинка</h3><p>Подарунки для великих команд, welcome-box.</p><div class="price-row"><span>Роздріб</span><span class="num">700 грн</span></div></div></div>
  ...
  <section class="block" id="how"><div class="wrap grid2">
    <div><p class="eyebrow">Як працюємо</p><h2 style="margin-top:10px">Від брифу до накладних</h2>
      <div class="steps" style="grid-template-columns:1fr;gap:18px;margin-top:22px">
        <div><h3>Бриф</h3><p>Клієнт, нагода, кількість, бюджет, дата. Досить одного листа.</p></div>
  ...
  <section class="form-block alt" id="request"><div class="wrap">
    ...
    <div>{form_html("f-ag", "Партнерство з агенцією", [
        ...
        ("project", "Клієнт, нагода, дата", "input", False, {"ph": "наприклад, IT-компанія, Новий рік, до 10 грудня", "full": True}),
        ("invoice", "Рахунок", "select:На агенцію|На клієнта|Ще не знаю", False, {}),
        ("note", "Коментар", "textarea", False, {"ph": "Бюджет на подарунок, листівка клієнта, побажання до принтів…"}),
    ], "")}</div>
  </div></section>'''
...
def speakers():
    body = hero("Конференції · форуми · корпоративні події", "Спікеру — не блокнот, а шовк",
        "Твіллі у фірмовій коробці замість чергового блокнота з логотипом: подарунок, який спікер надягне того ж вечора, а гості сфотографують.<span class=\"m-hide\"> Для спікерів, гостей і партнерів — один принт події або добірка.</span>",
        ...
        ("700 грн", "Резинка в коробочці — для гостей і welcome-box"),
  ...
  <section class="block" id="formats"><div class="wrap">
    ...
    <div class="grid4">
      ...
      <div class="card photo">{img("photo/scrunchie.jpg", "Резинка у коробочці", sizes="(max-width: 640px) 50vw, 25vw")}<div class="in"><h3>Гостям</h3><p>Резинка у фірмовій коробочці — маленька й помітна, таку не викидають.</p><div class="price-row"><span>Роздріб</span><span class="num">700 грн</span></div></div></div>
      ...
      <div class="card photo">{img("photo/paris-dots.jpg", "Шаль на плечах", sizes="(max-width: 640px) 50vw, 25vw")}<div class="in"><h3>Хедлайнеру</h3><p>Шаль 88 × 88 — подарунок, який запам’ятають.</p><div class="price-row"><span>Роздріб</span><span class="num">4 400 грн</span></div></div></div>
    </div>
  </div></section>
  ...
  <section class="block" id="stage" style="padding-top:0"><div class="wrap">
    <div class="stagebar">
      ...
      <figure>{img("photo/vyr-1.webp", "Шовкова хустка на білому пальті", sizes="(max-width: 640px) 50vw, 25vw")}<figcaption><b>Гостя</b><span>Хустка 44 або твіллі — у welcome-box</span></figcaption></figure>
      <figure>{img("photo/kolo-3.webp", "Хустка на плечах поверх пальта", sizes="(max-width: 640px) 50vw, 25vw")}<figcaption><b>Партнер події</b><span>Набір або хустка 65 × 65</span></figcaption></figure>
    </div>
  ...
  <section class="block" id="timeline"><div class="wrap">
    ...
    <div class="timeline">
      <div><p class="eyebrow">За 3 тижні</p><b>Запит і зразок</b><p>Кількість, формат, кольори події. Добірка принтів, розрахунок, зразок у роздріб. Або зберіть подарунок у <a href="b2b-atelier">3D-конструкторі</a>.</p></div>
...
def english():
    body = hero("Corporate gifts from Ukraine", "A gift that carries a story",
        "Scarves in 100% Italian silk with original prints by Ukrainian artist Svitlana Snizhko, hand-finished in Ukraine.<span class=\"m-hide\"> For teams abroad, partners and clients — in a signature yellow box with your card.</span>",
  ...
  <section class="block" id="why"><div class="wrap">
    ...
    <div class="grid4">
      <div class="card"><span class="k">01</span><h3>A story to tell</h3><p>Each print is a painting. The Singing Soul collection was created with artist Anna Klovak and the Ukrainian Society for the Protection of Birds: part of the proceeds goes towards nesting boxes for the European roller, a species listed in Ukraine’s Red Data Book.</p></div>
  ...
  <section class="block" id="how"><div class="wrap grid2">
    <div><p class="eyebrow">How it works</p><h2 style="margin-top:10px">From request to delivery</h2>
      <div class="steps" style="grid-template-columns:1fr;gap:18px;margin-top:22px">
        ...
        <div><h3>Selection and quote</h3><p>Prints and pieces for your budget with a quote; a sample can be ordered at retail from obiimy-world.com.</p></div>
        <div><h3>Approval</h3><p>Prints, the text of your card, the list of recipients. An invoice for payment by your company.</p></div>
  ...
  <section class="block alt" id="faq"><div class="wrap">
    <div class="head"><p class="eyebrow">Questions</p><h2>Frequently asked questions</h2></div>
    <div class="faq">
  ...
  <section class="block" id="proof"><div class="wrap grid2" style="align-items:start">
    <div><p class="eyebrow">Trust</p><h2 style="margin-top:10px">Where to find Obiimy</h2><p style="margin-top:14px;color:var(--ink2);max-width:36em">Obiimy is sold by retail partners in Ukraine and abroad, and its story has been covered by LIGA.net and INSIDER UA.</p></div>
    <div><div class="proof">
...
def inline(tpl, engine_file, form):
    ...
    states = [dict(s, price=s["old"] or s["price"], old="") for s in _states()]  # B2B: verified retail price, no promo
    return html.replace("/*STATES*/[]", json.dumps(states, ensure_ascii=False))
...
def garden_b2b():
    form = form_html("f-3d", "Корпоративні подарунки", BASE_FIELDS + [
        ...
        PAY,
        ("note", "Коментар", "textarea", False, {"ph": "Нагода, побажання до принтів, текст листівки…"}),
    ], "")
    return inline((ROOT / "b2b-garden.html").read_text(), "garden.js", form)

def atelier():
    form = form_html("f-at", "Конфігурація з конструктора подарунка", BASE_FIELDS + [
        ("qty", "Кількість подарунків", "number", False, {"ph": "з конструктора"}),
        ("deadline", "Коли потрібно", "select:До 10 грудня|До 20 грудня|Після свят|Інша дата", False, {}),
        ("delivery", "Доставка", "select:В офіс однією посилкою|Кожному на відділення Нової пошти|Ще не знаю", False, {}),
        PAY,
        ("note", "Коментар", "textarea", False, {"ph": "Нагода, терміни, побажання…"}),
    ], "")
    form = form.replace('<div class="actions">', '<input type="hidden" name="config" data-label="Конфігурація"><div class="actions">', 1)
