...
SAMPLE_URL = "https://obiimy.world/nabir-tvilli-845-ta-khustky-4444-natkhnennia/"
...
CSS = """
  * { box-sizing: border-box; min-width: 0; }
  ...
  .eyebrow { font-size: .74rem; letter-spacing: .2em; text-transform: uppercase; color: var(--ink3); font-weight: 600; }
  ...
  .hero .fine a { font-weight: 600; text-decoration: none; }
  ...
  .proof div { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 2fr); gap: 16px; padding: 14px 0; border-bottom: 1px solid var(--line); font-size: .95rem; }
  .proof b { font-family: var(--display); font-weight: 400; font-size: 1.15rem; }
  ...
  <section class="block{"" if first else " alt"}" id="proof"><div class="wrap grid2" style="align-items:start">
    ...
    <div>
      <div class="proof">
        ...
        <div><b>Be Brave · Канада, UFD London</b><span>Партнери за кордоном</span></div>
      ...
      <p class="quote" style="margin-top:22px">{quote}</p><p>Анна Мелешак · відгук покупниці про якість, obiimy.world</p>
    </div>
  </div></section>'''
...
def form_html(pid, subject, fields, note):
    ...
    for name, label, kind, req, extra in fields:
        ...
        err = f'<span class="err">{extra.get("err", "Заповніть це поле")}</span>' if req else ""
        if kind.startswith("select:"):
            ...
            ctl = f'<input name="{name}" data-label="{label}" type="{t}"{mn}{ph}{ac}{r}>'
    ...
    return f'''
      ...
      <div class="done" id="{pid}-done" hidden>
        <h3>Лист підготовлено</h3>
      ...
      (function () {{
        ...
        f.addEventListener('submit', function (e) {{
          ...
          f.hidden = true; done.hidden = false; done.scrollIntoView({{ block: 'start', behavior: 'smooth' }});
...
<meta property="og:image" content="{SITE_URL}/{page["og"]}">
...
<header class="nav"><div class="wrap">
  <a class="logo" href="{skin["landing"]}" aria-label="Obiimy — головна"><img src="{logo}" alt="Obiimy" width="120" height="26"></a>
...
<script>
(function () {{
  var s = document.getElementById('sticky'), cta = document.querySelector('.hero .cta'), req = document.getElementById('request');
  if (!('IntersectionObserver' in window)) {{ s.classList.add('on'); return; }}
  var passed = false, inForm = false;
  function upd() {{ s.classList.toggle('on', passed && !inForm); }}
  new IntersectionObserver(function (es) {{ es.forEach(function (x) {{ passed = !x.isIntersecting && x.boundingClientRect.top < 0; }}); upd(); }}).observe(cta);
  new IntersectionObserver(function (es) {{ es.forEach(function (x) {{ inForm = x.isIntersecting; }}); upd(); }}, {{ threshold: 0.15 }}).observe(req);
}})();
</script>
</body>
</html>
...
def hero(eyebrow, h1, lead, cta1, cta2, cta2_href, fine, photo, alt, tag):
    return f'''
...
def newyear():
    ...
    body = hero("Новий рік 2027 · корпоративні подарунки", "Подарунок, який не залишать у шухляді",
                "Шовкова річ ручної обробки у фірмовій жовтій коробці з листівкою від вашої компанії. Один принт на всю команду або добірка під кожну людину — від резинки за 700 грн до набору з трьох твіллі.",
                "Отримати добірку і розрахунок", "Замовити зразок у роздріб", SAMPLE_URL,
                "Відповідаємо з добіркою принтів і цінами під ваш тираж.", "photo/box-dots.jpg", "Фірмова жовта коробка Obiimy з шовковою хусткою в горох", "Кожен подарунок — у фірмовій коробці з тонким папером і листівкою") + facts_html([
  ...
  <section class="block alt" id="timeline"><div class="wrap">
    ...
    <div class="timeline">
      <div><p class="eyebrow">Крок 1</p><b>Жовтень</b><p>Запит і зразок. Надсилаємо добірку принтів, розрахунок на тираж і рахунок для погодження бюджету.</p></div>
    ...
    <p class="note">Запити після 15 листопада беремо в роботу з принтами, що є в наявності на складі.</p>
  </div></section>

  <section class="block" id="how"><div class="wrap">
    <div class="head"><p class="eyebrow">Як це працює</p><h2>П’ять кроків до подарунка</h2></div>
    <div class="steps">
      <div><h3>Запит</h3><p>Форма нижче або дзвінок. Досить кількості й орієнтовного бюджету.</p></div>
      <div><h3>Добірка</h3><p>Принти й речі під ваш бюджет, з розрахунком на тираж.</p></div>
      <div><h3>Зразок</h3><p>Один набір у роздріб сьогодні — потримати в руках і показати команді. <a href="{SAMPLE_URL}">Замовити зразок →</a></p></div>
      <div><h3>Пакування</h3><p>Кожна річ у коробці, з листівкою та вашим текстом.</p></div>
      <div><h3>Відправка</h3><p>Нова пошта по Україні, за кордон — за тарифами перевізника.</p></div>
    </div>
  </div></section>
  ...
  <section class="block" id="after"><div class="wrap">
    <div class="head"><p class="eyebrow">Після Нового року</p><h2>Один список адрес — і подарунки їдуть самі</h2><p class="sub">8 березня, дні народження, welcome-box для нових співробітників, подяка клієнтам. Для компаній, які дарують не раз на рік, є річна програма.</p></div>
    <div class="grid3">
  ...
  {proof_html("Кому ми вже продаємо", "Obiimy продається у роздрібних партнерів в Україні та за кордоном, а історію бренду розповідали LIGA.net та INSIDER UA.")}
  ...
  <section class="form-block alt" id="request"><div class="wrap">
    ...
    return dict(slug="b2b-newyear", skin="maison", title="Корпоративні подарунки на Новий рік 2027 — шовкові хустки Obiimy",
...
def horeca():
    cases = [
        ...
        ("Ресторани та бари", "Паше 44 × 44 як нашийна хустка хостес — один принт на команду, що не м’яється за вечір.", "photo/enerhiia-knot.webp", "Шовкова хустка вузлом на шиї"),
        ("Авіація та бізнес-джети", "Хустка 65 × 65 у кольорах авіакомпанії — класика бортової уніформи; твіллі в нагрудній кишені — для чоловічої частини команди.", "photo/riviera-car.jpg", "Хустка на голові за кермом"),
        ("Банки та private banking", "Стримані монограмні принти для менеджерів; та сама хустка — як подарунок клієнту.", "photo/makiv-knot.webp", "Хустка на білій сорочці"),
    ]
    ...
    body = hero("HoReCa · сервіс · уніформа", "Шовк у вашому дрес-коді",
                "Одна деталь із 100% італійського шовку робить уніформу впізнаваною: хустка на шиї хостес, твіллі на рецепції готелю, маска для сну в номері VIP-гостя. Принти з колекцій Obiimy — під кольори вашого бренду.",
  ...
  <section class="block" id="cases"><div class="wrap">
    <div class="head"><p class="eyebrow">Сценарії</p><h2>Де шовк працює на бренд</h2><p class="sub">Чотири формати уніформи, з якими ми працюємо. У кожному — стандартні речі з каталогу Obiimy, тож зразок можна замовити сьогодні й приміряти на команді за кілька днів.</p></div>
  ...
  <section class="block alt" id="range"><div class="wrap">
    ...
    <div class="grid3">
      <div class="card photo">{img("photo/riviera-red.jpg", "Твіллі на зап’ясті", sizes="(max-width: 640px) 100vw, 33vw")}<div class="in"><h3>Паше 44 × 44</h3><p>Нашийна хустка або в кишеню жакета. Найпрактичніший розмір для щоденної зміни.</p><div class="price-row"><span>Роздріб</span><span class="num">від 1 600 грн</span></div></div></div>
      ...
      <div class="card photo">{img("photo/riviera-boat.jpg", "Хустка 65 × 65 на палубі", sizes="(max-width: 640px) 100vw, 33vw")}<div class="in"><h3>Хустка 65 × 65</h3><p>Класичний формат для авіації та рецепцій: вузол, пов’язка, на плечі.</p><div class="price-row"><span>Роздріб</span><span class="num">від 3 200 грн</span></div></div></div>
    </div>
  ...
  <section class="block" id="calc"><div class="wrap grid2" style="align-items:start">
    ...
    <div class="calc" id="calcbox">
      <div class="row">
        ...
        <label>Річ<select id="c-item"><option value="1600">Паше 44 × 44 · 1 600 грн</option><option value="1600">Твіллі 84 × 5 · 1 600 грн</option><option value="3200">Хустка 65 × 65 · 3 200 грн</option></select></label>
        <label>На людину<select id="c-per"><option value="1">1 річ</option><option value="2" selected>2 речі (зміна + прання)</option></select></label>
      </div>
      <div class="out"><b class="num" id="c-out">38 400 грн</b><span>орієнтовно за роздрібними цінами · <span id="c-pcs">24</span> шт.</span></div>
    </div>
  </div></section>
  <script>
  (function () {{
...
def wholesale():
    ...
    body = hero("Для магазинів, бутиків і корнерів", "Obiimy на вашій полиці",
                "Український бренд шовкових хусток із власними принтами, який уже продається в INTERTOP, Hram, Be Brave (Канада) та UFD London. Пропонуємо партнерам готову вітрину: асортимент, коробки, фотоконтент й історію, яку легко розповісти покупцю.",
        ...
        ("4 партнери", "INTERTOP, Hram, Be Brave · Канада, UFD London"),
  ...
  {proof_html("Кому ми вже продаємо", "Obiimy продається у роздрібних партнерів в Україні та за кордоном, а історію бренду розповідали LIGA.net та INSIDER UA.", first=True)}
  ...
  <section class="block alt" id="partner"><div class="wrap grid2">
  ...
  <section class="block" id="formats"><div class="wrap">
    ...
    <div class="faq" style="margin-top:32px">
      <details><summary>Мінімальна партія і знижка</summary><p>Залежать від формату й категорій — надсилаємо в оптовому прайсі після запиту.</p></details>
      ...
      <details><summary>Ексклюзив у місті</summary><p>Обговорюємо для корнерів і міжнародних партнерів залежно від обсягу.</p></details>
      {DOCS_FAQ}
    </div>
  </div></section>

  <section class="form-block alt" id="request"><div class="wrap">
    <div class="contact"><p class="eyebrow">Напишіть нам</p><h2>Запросити оптовий прайс</h2><p>Розкажіть про магазин і місто — надішлемо прайс, каталог із фото та умови для вашого формату.</p><p class="big"><a href="{PHONE_HREF}">{PHONE}</a></p><p><a href="mailto:{MAIL}">{MAIL}</a></p></div>
    ...
    return dict(slug="b2b-wholesale", skin="journal", title="Шовкові хустки оптом для магазинів — Obiimy",
                ...
                og="photo/paris-dots.jpg", nav=[("Довіра", "proof"), ("Асортимент", "range"), ("Для партнера", "partner"), ("Формати", "formats"), ("Контакт", "request")],
...
def calendar():
    months = [
        ...
        ("Постійно", "Подяка клієнтам", "Закриття великої угоди, річниця співпраці — набір «Натхнення» або хустка 88 × 88.", "photo/probudzhennia-2.webp", "Хустка «Пробудження» на плечах"),
        ("Дата компанії", "Річниця бренду", "Один принт на всю команду — і фото на згадку в один день.", "img/life3.webp", "Хустка на голові, білий жакет"),
    ]
    ...
    body = hero("Річна програма · корпоративні подарунки", "Один список адрес — і подарунки їдуть самі",
                "Для компаній, які дарують не раз на рік. Погоджений бюджет, добірка принтів під кожну нагоду — від Нового року до welcome-box — і відправка кожному адресату за графіком, без вашої участі.",
                "Скласти річний план подарунків", "Календар нагод", "#year",
                "Від 700 грн за подарунок. Список адресатів — від вас, усе інше — від нас.", "photo/paris-green.jpg", "Зелена шовкова хустка на бежевому жакеті", "Вісім нагод на рік · один бюджет · один список") + facts_html([
  ...
  <section class="block" id="year"><div class="wrap">
    <div class="head"><p class="eyebrow">Календар</p><h2>Рік у восьми коробках</h2><p class="sub">Нагоди, з якими до нас приходять компанії, і що ми для них пропонуємо. Все — стандартні речі з каталогу Obiimy за роздрібними цінами від 700 до 4 800 грн.</p></div>
